...
# Combine all simulation files into a list
...
# Get observations
obs <- read.csv(paste0(observation.file))[,-c(1)]
...
# STEP 3: DEFINE INVERSION WINDOWS
#---------------------------------------------------------------------------
...
for (t in 1:n.ints){
  ...
  this.mask <- times %within% interval(int.breaks[t], int.breaks[t+1])
...
for (t in 1:n.ints){
  ...
  if (length(all.preds.to.compare) > 4){
    ...
    rate.est.all.events[t] <- mean(q.vals, na.rm = T) 
...
# Save output
to.save <- data.frame(time = int.breaks[-length(int.breaks)], 
                      localization.estimate = loc.est.all.events,
                      rate.estimate = rate.est.all.events,
                      rate.estimate.lower = error.lower.all.events,
                      rate.estimate.upper = error.upper.all.events)

# Truth: tanks @ 2.1 kg/hr

# Save results
write.csv(to.save, paste0(output.file.path, "DLQ_demo_output.csv"))
